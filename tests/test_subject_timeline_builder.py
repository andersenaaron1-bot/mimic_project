...
def test_routed_transfer_to_discharge_marks_discharge_like_transition() -> None:
    t0 = datetime(2024, 1, 1, 8, 0, 0)
    events = [SimpleNamespace(code="TRANSFER_TO//discharge//UNKNOWN", time=t0)]
    db = DummyDB({1: DummySubject(events)})
    ...
    tok = tokens[0]
    assert tok.cat_attrs["transition_action_id"] == TRANSITION_ACTION_TO_ID["close_open"]
    assert tok.cat_attrs["transition_transfer_like"] == 1
    assert tok.cat_attrs["transition_discharge_like"] == 1
    assert tok.window_hook == "episode"
